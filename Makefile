...
_JSOND:=		./data/redemdata.$(_DATE).data
_JSONDL:=		$(_JSOND).json
...
REDDIT_USERNAME=is_this_valid
REDDIT_USERNAME2=is_this_valid2
...
backup:
	...
	$(_REDEM_BIN) --verbose \
		--backup \
		--json=$(_JSOND).$(REDDIT_USERNAME).json \
		--username=$(REDDIT_USERNAME) \
		$(BACKUP_OPTS) && \
	$(_REDEM_BIN) --verbose \
		--backup \
		--json=$(_JSOND).$(REDDIT_USERNAME2).json \
		--username=$(REDDIT_USERNAME2) \
		$(BACKUP_OPTS) && \
